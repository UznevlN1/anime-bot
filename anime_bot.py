...
# Pyrogramning sync moduli import vaqtida asyncio.get_event_loop() ni chaqiradi.
# Python 3.12+da agar hali joriy event loop oʻrnatilmagan boʻlsa, bu xatolik beradi
# ("There is no current event loop in thread"). Shu sabab pyrogramni import qilishdan
# oldin main thread uchun loop qoʻlda oʻrnatib qoʻyamiz.
import asyncio as _asyncio_bootstrap
try:
    _asyncio_bootstrap.get_event_loop()
except RuntimeError:
    _asyncio_bootstrap.set_event_loop(_asyncio_bootstrap.new_event_loop())
...
async def start_web_server():
    ...
    async def webapp_stream(request):
        ...
        try:
            ...
            logger.error(f"[stream] get_messages xato: {e}")
        ...
        offset_chunk = start // STREAM_CHUNK_SIZE
        first_cut = start % STREAM_CHUNK_SIZE
        sent = 0
        try:
            async for chunk in pyro.stream_media(msg, offset=offset_chunk):
                if sent == 0 and first_cut:
            ...
            logger.error(f"[stream] uzatish xatosi: {e}")
        return resp
