...
# Premium emojilar (foydalanuvchi tomonidan yuborilgan, faqat caption/matn ichida ishlaydi — tugmalarda EMAS)
def pe(emoji_id, fallback):
    return f'<tg-emoji emoji-id="{emoji_id}">{fallback}</tg-emoji>'

PE_DOWNLOAD  = pe("4965313018326942268", "⬇️")
PE_ANNOUNCE  = pe("4967957395331351254", "📢")
PE_MAIL      = pe("4967508210471666645", "✉️")
PE_BACK      = pe("4970176665062736422", "◀️")
PE_GLOBE     = pe("4969799614178788098", "🌐")
PE_HOME      = pe("4970038633403777664", "🏠")
PE_PLUS      = pe("5030749344752468962", "➕")
PE_LEFT      = pe("4972453139463537420", "⬅️")
PE_RIGHT     = pe("4969851488793788974", "➡️")
PE_PLUS2     = pe("5030787243543888610", "➕")
PE_UP        = pe("4967897119760319376", "⬆️")
PE_MIC       = pe("4969883048213480204", "🎙️")
PE_MINUS     = pe("4970086827231806362", "➖")
PE_IMAGE     = pe("5033108527338488459", "🖼")
PE_NOTE      = pe("5033080906403808074", "📝")
PE_CANCEL    = pe("5032973497861669622", "❌")
PE_FIRE      = pe("4970107898341360413", "🔥")
PE_SEARCH    = pe("5429571366384842791", "🔎")
PE_EDIT      = pe("5879841310902324730", "✏️")
PE_USER      = pe("5771887475421090729", "👤")
PE_DIAMOND   = pe("6028530359975548369", "💎")
PE_DOC       = pe("5032963696746300412", "📃")

PREMIUM_EMOJI_DOWNLOAD = PE_DOWNLOAD  # eski nom bilan ham ishlatilishi mumkin
...
def anime_card_text(anime):
    return (
        ...
        f"🎭 Janr: {anime['genre']}\n"
        f"{PREMIUM_EMOJI_DOWNLOAD} Yuklab olish uchun pastdagi tugmani bosing\n\n"
        f"📝 {anime['description']}"
    )
...
async def start_handler(message: Message, state: FSMContext):
    ...
    if not u:
        await message.answer(
            ...
            f"{PE_NOTE} Iltimos, quyidagi qoidalarni o'qib chiqing va qabul qiling.",
...
async def search_callback(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        f"{PE_SEARCH} Anime nomini yozing (to'liq nom):",
...
async def episode_handler(call: CallbackQuery):
    ...
    protect = await asyncio.to_thread(db.get_setting, "content_protect") == "1"
    video_kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="◀️ Orqaga", callback_data=f"backcard_{ep['anime_id']}", style="primary")],
        [InlineKeyboardButton(text="🏠 Bosh menu", callback_data="main_menu", style="primary")],
    ])
    await bot.copy_message(
        call.message.chat.id,
...
async def admin_add(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(f"{PE_IMAGE} Anime rasmini yuboring:", reply_markup=admin_back(), parse_mode="HTML")
...
async def banner_add(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(f"{PE_IMAGE} Banner rasmini yuboring:", reply_markup=admin_back(), parse_mode="HTML")
...
async def webapp_profile(request):
    ...
    return web.json_response({
        ...
        "is_premium": False,  # hozircha premium tizimi yoʻq, kelajakda db ustuniga ulanadi
...
async def webapp_get_comments(request):
    ...
    if not status["subscribed"]:
        return web.json_response({"error": "not_subscribed", "channels": status["channels"]}, status=403)
    anime_id = int(request.match_info["anime_id"])
    comments = await asyncio.to_thread(db.get_comments, anime_id, 50)
...
async def webapp_add_comment(request):
    ...
    new_id = await asyncio.to_thread(db.add_comment, anime_id, user_id, username, text)
