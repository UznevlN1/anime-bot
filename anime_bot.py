...
async def episode_handler(call: CallbackQuery):
    ...
    # Video ostiga barcha qismlar tugmalarini ham qo'shamiz — shu orqali
    # foydalanuvchi kartochkaga qaytmasdan, to'g'ridan-to'g'ri boshqa qismga o'tishi mumkin.
    all_eps = await asyncio.to_thread(db.get_episodes, ep["anime_id"])
    ep_buttons = []
    row = []
    for e in sorted(all_eps, key=lambda x: x["episode_number"]):
        style = "success" if e["id"] == episode_id else "primary"
        row.append(InlineKeyboardButton(text=f"{e['episode_number']}-qism", callback_data=f"ep_{e['id']}", style=style))
        if len(row) == 4:
    ...
    video_kb = InlineKeyboardMarkup(inline_keyboard=ep_buttons + [
