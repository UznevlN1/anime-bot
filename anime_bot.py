...
# ===================== YORDAMCHI =====================
_SUB_CACHE = {}  # user_id -> (bool, expires_at_monotonic)
_SUB_CACHE_TTL = 60  # soniya — shu muddat ichida qayta Telegram'dan so'ralmaydi

async def check_subscription(user_id, use_cache=True):
    now = time.monotonic()
    if use_cache:
    ...
    if premium["is_premium"]:
        _SUB_CACHE[user_id] = (True, now + _SUB_CACHE_TTL)
        return True
    ...
    async def _check_one(ch):
        try:
            ...
            # Bu — KONFIGURATSIYA xatosi (foydalanuvchining aybi emas):
            # bot shu kanalda admin emas, channel_id noto'g'ri yoki kanal
            # o'chirilgan/yopilgan. Bunda hammani bloklash noto'g'ri,
            # shuning uchun bu kanalni "obuna bor" deb hisoblab o'tkazib yuboramiz,
            # lekin adminga aniq xato yozib qo'yamiz — tuzatish uchun.
            logger.error(
                f"[obuna] SOZLAMA XATOSI: '{ch['channel_id']}' kanalini tekshirib bo'lmadi — "
                f"bot bu kanalda ADMIN emasmi yoki ID notogrimi, tekshiring! Xato: {e}"
            )
            return True
        except Exception as e:
            logger.warning(
                f"[obuna] {ch['channel_id']} kanalini tekshirib bo'lmadi (user={user_id}): {e}"
            )
            return False
...
async def check_sub_handler(call: CallbackQuery, state: FSMContext):
    await call.answer()
    subscribed = await check_subscription(call.from_user.id, use_cache=False)
    if subscribed:
        await call.message.edit_text(
...
# ===================== /OBUNATEST (diagnostika) =====================
@dp.message(Command("obunatest"))
async def obunatest_handler(message: Message):
    """Faqat admin uchun: har bir majburiy kanalni haqiqatda tekshirib,
    bot get_chat_member so'rovini yubora olyaptimi-yo'qmi — aniq matn bilan ko'rsatadi.
    Foydalanish: /obunatest  yoki  /obunatest 123456789 (boshqa user_id uchun)"""
    if message.from_user.id != ADMIN_ID:
    ...
    channels = await asyncio.to_thread(db.get_channels)
    if not channels:
        await message.answer("⚠️ Bazada hech qanday majburiy kanal yo'q (Kanallar bo'limi bo'sh).")
        return

    lines = [f"🧪 Tekshirilayotgan user_id: <code>{target_id}</code>\n"]
    for ch in channels:
        cid = ch["channel_id"]
        try:
            ...
            lines.append(
                f"✅ <b>{ch['channel_name']}</b> ({cid})\n"
                f"   Holat: <code>{member.status}</code> → "
                f"{'obuna bor' if ok else 'obuna yoq'}"
            )
        except Exception as e:
            lines.append(
                f"❌ <b>{ch['channel_name']}</b> ({cid})\n"
                f"   XATO: <code>{type(e).__name__}: {e}</code>"
            )
...
async def webapp_access_status(user_id: int, fresh: bool = False):
    ...
    subscribed = await check_subscription(user_id, use_cache=not fresh)
    channels_out = []
    if not subscribed:
        channels = await asyncio.to_thread(db.get_channels)
        for ch in channels:
...
async def webapp_check_access(request):
    ...
    status = await webapp_access_status(user_id, fresh=True)
...
async def webapp_get_history(request):
    try:
        ...
        return web.json_response({"error": "notogri sorov"}, status=400)
    ids = await asyncio.to_thread(db.get_recent_anime_ids, user_id, 8)
    return web.json_response({"ids": ids})
