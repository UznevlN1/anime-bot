...
async def announce_to_channel(anime, kind="anime", episode_number=None, episode_id=None):
    """Yangi anime yoki yangi qism qo'shilganda STORAGE_CHANNEL'ga e'lon post qiladi.
    ...
    try:
        ...
        if photo_id:
            await bot.send_photo(STORAGE_CHANNEL, photo_id, caption=caption, reply_markup=kb, parse_mode="HTML")
        else:
            await bot.send_message(STORAGE_CHANNEL, caption, reply_markup=kb, parse_mode="HTML")
    except Exception as e:
        logger.error(f"[announce_to_channel] e'lon yuborilmadi: {e}")

# ===================== YORDAMCHI =====================
_SUB_CACHE = {}  # user_id -> (timestamp, bool) — check_subscription natijasini qisqa vaqt eslab qolish uchun
_SUB_CACHE_TTL = 300  # 5 daqiqa
...
async def check_subscription(user_id):
    ...
    # Eslatma: bu funksiya deyarli HAR BIR tugma bosilganda (guard_access orqali)
    # chaqiriladi. Ilgari har bir majburiy kanal uchun Telegramga KETMA-KET so'rov
    # yuborilardi — 2-3 ta kanal bo'lsa, har bir klikda soniyalab kechikish paydo
    # bo'lardi. Endi: (1) natija qisqa muddat (5 daqiqa) keshlanadi, shuning uchun
    # bir marta obuna tasdiqlansa qayta-qayta Telegramga murojaat qilinmaydi;
    # (2) kesh muddati o'tgan/hali tekshirilmagan holatda ham, kanallar KETMA-KET
    # emas — barchasi PARALLEL (asyncio.gather) tekshiriladi.
    ...
    subscribed = all(results)
    _SUB_CACHE[user_id] = (time.time(), subscribed)
    return subscribed
...
async def ch_add_done(message: Message, state: FSMContext):
    ...
    _SUB_CACHE.clear()  # kanallar ro'yxati o'zgardi — barcha foydalanuvchilar uchun qayta tekshirilsin
