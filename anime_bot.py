...
async def check_subscription(user_id):
    ...
    for ch in channels:
        try:
            ...
            # "user not found" / "participant" — foydalanuvchi bu kanalga
            # hech qachon a'zo bo'lmagan (hatto ko'rinmagan), demak obuna
            # emas. Buni jim o'tkazib yuborish orqali majburiy obuna
            # butunlay chetlab o'tilardi. Boshqa turdagi xatolar (masalan
            # kanal noto'g'ri sozlangan/bot admin emas) foydalanuvchini
            # bloklamasligi uchun pastdagi umumiy except'ga tushadi.
            msg = str(e).lower()
            if "user not found" in msg or "participant" in msg:
                return False
        except Exception:
            pass
    return True
...
def _episode_locked(episode, user_id, prices, is_premium):
...
async def webapp_profile(request):
    user_id = _webapp_user_id(request)
    u, channel_url, support_url, premium, prices = await asyncio.gather(
...
async def webapp_anime_detail(request):
    ...
    if episodes:
        prices = await premium_settings()
        is_premium = False
        if user_id != ADMIN_ID and prices["enabled"] and prices["early_hours"] > 0:
            premium_status = await asyncio.to_thread(db.get_premium_status, user_id)
            is_premium = premium_status["is_premium"]
        for ep in episodes:
            ep["is_locked"] = _episode_locked(ep, user_id, prices, is_premium)
