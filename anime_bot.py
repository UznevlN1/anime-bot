...
def video_episodes_keyboard(episodes, anime_id, page=0):
    ...
    kb = episodes_keyboard(episodes, anime_id, page)
    new_rows = []
    for row in kb.inline_keyboard:
        new_row = []
        for btn in row:
            if btn.callback_data and btn.callback_data.startswith("eps_"):
                new_row.append(InlineKeyboardButton(text=btn.text, callback_data=btn.callback_data.replace("eps_", "epv_", 1)))
            else:
                new_row.append(btn)
...
async def episodes_page_video(call: CallbackQuery):
    ...
    episodes = await asyncio.to_thread(db.get_episodes, anime_id)
    await call.message.edit_reply_markup(
        reply_markup=video_episodes_keyboard(episodes, anime_id, page)
    )
...
async def episode_handler(call: CallbackQuery):
    ...
    page = idx // 6
    video_kb = video_episodes_keyboard(all_eps_sorted, ep["anime_id"], page)
