...
async def check_subscription(user_id):
    ...
    # bo'lardi. Endi: (1) faqat "OBUNA BOR" natijasi qisqa muddat (5 daqiqa)
    # keshlanadi — "obuna yo'q" natijasi HECH QACHON keshlanmaydi, aks holda
    # foydalanuvchi hozirgina kanalga obuna bo'lsa ham, eski keshlangan "obuna
    # yo'q" javobi tufayli yana bir necha daqiqa noto'g'ri bloklanib qolar edi;
    # (2) kanallar KETMA-KET emas — barchasi PARALLEL (asyncio.gather) tekshiriladi.
    cached = _SUB_CACHE.get(user_id)
    if cached and cached[1] and (time.time() - cached[0]) < _SUB_CACHE_TTL:
        return True
