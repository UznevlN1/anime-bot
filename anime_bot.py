...
async def start_web_server():
    ...
    async def webapp_stream(request):
        ...
        try:
            ...
            logger.warning(f"[stream] birinchi urinish muvaffaqiyatsiz ({e}), get_chat orqali qayta urinilmoqda...")
            try:
...
async def main():
    ...
    if STREAM_ENABLED:
        try:
            await pyro.start()
            # Pyrogram (MTProto) botning a'zo bo'lgan maxfiy kanallarini "tanishi" uchun
            # kamida bir marta dialoglar ro'yxatini o'qib chiqishi kerak — aks holda
            # keyinchalik ID orqali xabar olishga uringanda "Peer id invalid" xato beradi.
            try:
                async for _ in pyro.get_dialogs():
                    pass
            except Exception as e:
                logger.warning(f"Pyrogram get_dialogs xato (davom etamiz): {e}")
